...
class ReprMethodTests(unittest.TestCase):
    ...
    def test_repr_no_spurious_warnings(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            repr_complex = repr(hydrated_complex)
            repr_simple = repr(hydrated_simple)
            repr_array = repr(hydrated_array)
            repr_primitive = repr(hydrated_primitive)
            
            # Call __repr__ on nested objects
            repr_nested_object = repr(hydrated_complex.user)
            repr_nested_array = repr(hydrated_complex.items)
            repr_deeply_nested = repr(hydrated_complex.user.preferences)
            
            # No warnings should be generated from __repr__ calls
            repr_warnings = [warning for warning in w if issubclass(warning.category, PyHydrateWarning)]
            assert len(repr_warnings) == 0, f"Unexpected warnings from __repr__: {[str(w.message) for w in repr_warnings]}"
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            for data, description in test_cases:
                with self.subTest(description=description):
                    ...
                    # Should contain PyHydrate identifier
                    assert "PyHydrate" in repr_output or "Primitive" in repr_output or "Object" in repr_output or "Array" in repr_output
            
            # No warnings should be generated
            repr_warnings = [warning for warning in w if issubclass(warning.category, PyHydrateWarning)]
            assert len(repr_warnings) == 0, f"Unexpected warnings: {[str(w.message) for w in repr_warnings]}"
    ...
    def test_repr_with_debug_mode(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            repr_warnings = [warning for warning in w if issubclass(warning.category, PyHydrateWarning)]
            assert len(repr_warnings) == 0, f"Debug mode __repr__ generated warnings: {[str(w.message) for w in repr_warnings]}"
    ...
    def test_str_no_spurious_warnings(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            str_output = str(hydrated)
            str_nested = str(hydrated.user)
            str_array = str(hydrated.items)
            str_primitive = str(hydrated.user.name)
            
            # No warnings should be generated from __str__ calls
            str_warnings = [warning for warning in w if issubclass(warning.category, PyHydrateWarning)]
            assert len(str_warnings) == 0, f"Unexpected warnings from __str__: {[str(w.message) for w in str_warnings]}"
    ...
    def test_repr_vs_actual_api_warnings(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            invalid_result = hydrated("invalid_call_type")
            
            # Filter warnings
            api_warnings = [warning for warning in w if issubclass(warning.category, APIUsageWarning)]
            
            # Should have exactly one warning (from the invalid call)
            assert len(api_warnings) == 1, f"Expected 1 API warning, got {len(api_warnings)}"
    ...
    def test_repr_with_notation_classes_directly(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            api_warnings = [warning for warning in repr_warnings if issubclass(warning.category, APIUsageWarning)]
            assert len(api_warnings) == 0, f"Unexpected APIUsageWarnings from __repr__: {[str(w.message) for w in api_warnings]}"
    ...
    def test_repr_memory_efficiency(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            repr_warnings = [warning for warning in w if issubclass(warning.category, PyHydrateWarning)]
            assert len(repr_warnings) == 0, f"Large data __repr__ generated warnings: {[str(w.message) for w in repr_warnings]}"

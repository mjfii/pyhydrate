...
class ArrayEdgeCasesTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.nested_arrays = PyHydrate([[], ["a"], ["b", "c"], [1, 2, 3, 4, 5]])
        self.mixed_array = PyHydrate(
            ["string", 42, 3.14, True, None, {"object": "value"}, [1, 2, 3]]
        )
    ...
    def test_no_warnings_from_bounds_checking(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(pyhydrate_warnings) == 0, (
                f"Unexpected warnings from indexing: {[str(w.message) for w in pyhydrate_warnings]}"
            )

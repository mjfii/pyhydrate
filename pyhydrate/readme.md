...
- **`pyhydrate.py`** - The `PyHydrate` class. Main entry point that accepts dicts, lists, JSON/YAML/TOML strings, file paths, and iterable types (`tuple`, `set`, `frozenset`, `range` — converted to list) and wraps them for dot notation access. Immutable inputs (`tuple`, `frozenset`) emit `ImmutableConversionWarning`; unsupported types raise `TypeError`. Supports mutation via `__setattr__`/`__setitem__`, deletion via `__delattr__`/`__delitem__`, and file persistence via `save(path, output_format=, original_keys=)`. Supports saving to local files and remote cloud storage (S3, GCS, ADLS) via `fsspec`. Auto-promotes to `NotationObject` on first write when constructed empty. Inherits from `NotationBase`.
